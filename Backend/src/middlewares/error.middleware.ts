...
const handleMessage = (message: string, res: express.Response) => new ApiErrors( message, 401)

const globalErrors = (err: any, req: express.Request, res: express.Response, next: express.NextFunction) => {
    err.statusCode = err.statusCode || 500;
    err.status = err.status || "Error";
    if( err.name == 'TokenExpiredError' || err.name == 'JsonWebTokenError') err = handleMessage(`${req.__('session_expired')}`, res)
