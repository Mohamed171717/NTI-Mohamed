    ...
    createOne = [ 
        ...
        body('subcategory')
            ...
            .custom(async (val: string, {req}) => {
                ...
                // else if( subcategory?.category._id! != req.body.category) throw new Error(`update products error`);
                return true;
            })
        , validatorMiddleware]
        ...
        body('category').optional()
            ...
            .custom(async (val: string, {req}) => {
                ...
                if (!category) throw new Error(`${req.__('validation_value')}`)
                    ...
                    //     if( product?.subcategory._id!.toString()!== req.body.subcategory.toString()) throw new Error(`failed products subcayegory does not belong to this category!`); // request subcategory
                    // }
                return true;
            }),
        body('subcategory').optional()
            ...
            .custom(async (val: string, {req}) => {
                ...
                if (!subcategory ) throw new Error(`${req.__('validation_value')}`)
                    else if( subcategory?.category._id != req.body.category) throw new Error(`${req.__('failed_update')}`); // ? => mean optional
                    // else if( subcategory.category._id!.toString() !== req.body.category.toString()) throw new Error(`${req.__('failed_update')}`);
                    // else {
                    //     const product = await productsSchema.findById( req.params!.id );
                    //     if ( subcategory.category.name != product?.name ) throw new Error(`update product error!!`);
                    // }
                return true;
            })
        , validatorMiddleware]
